...
function getPDO($debugged = null) {
    ...
    if ($debugged === null ? DEBUG_MODE : $debugged) {
        ...
        $dbh =  new LoggedPDO( "mysql:host=$dbhost;dbname=$db;charset=utf8", $dbuser, $dbpass, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
    } else {
        FB::log("Kapalı", "Veritabanı Debug Modu:");
        $dbh = new PDO( "mysql:host=$dbhost;dbname=$db;charset=utf8", $dbuser, $dbpass, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_SILENT));
    } 
