...
<div class="maincontainer">
    ...
    $db = getPDO();
    $gecersizid = false;
    $kayitvarmi = $db->prepare('SELECT kid FROM kaydedilenler LIMIT 1');
    $kayitvarmi->execute();
    if ($kayitvarmi->rowCount() === 0)
        echo "Şu ana kadar bir şey kaydedilmemiş. Ayıp!";
    else {
    ...
    if (!$gecersizid) {
        ...
        <?php echo $textile->textileRestricted($siir['kayit']); ?>
        <div class="sharebox">
            <div class="inner">
                <ul class="share">
                    ...
                    <li style="float:right; margin-top:10px;font-size:9pt;font-family:Verdana,Helvetica,Arial,sans-serif;"><?php if ($nick === "" || empty($nick)) echo "";
                else echo "<a href='profil.php?kid=" . $siir['uid'] . "'>$nick</a> kaydetti."; ?></li>
                </ul>
            </div>
        </div>
        <?php
